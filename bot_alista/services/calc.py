...
class CustomsCalculator:
    ...
    def calculate_clearance_tax(self):
        """Calculate customs clearance fee in RUB.

        Priority:
        1) Tariff config new schema: tariffs.clearance_fee.base_rub
        2) Tariff config legacy schema: tariffs.base_clearance_fee
        3) Fallback ranges based on customs value in RUB
        """
        tariffs = (self.config or {}).get("tariffs", {})

        # 1) New schema: nested dict with base amount in RUB
        fee_cfg = tariffs.get("clearance_fee")
        if isinstance(fee_cfg, dict):
            base_raw = fee_cfg.get("base_rub")
            try:
                ...
                logger.warning("Invalid clearance_fee.base_rub value in config; falling back")

        # 2) Legacy schema: flat number at tariffs.base_clearance_fee
        legacy_raw = tariffs.get("base_clearance_fee")
        try:
            if legacy_raw is not None:
                base = float(legacy_raw)
                logger.info(f"Customs clearance fee (legacy config): {base} RUB")
                return base
        except (TypeError, ValueError):
            logger.warning("Invalid base_clearance_fee value in config; falling back")

        # 3) Fallback to static ranges by customs value in RUB
        try:
            price_rub = self.convert_to_local_currency(self.vehicle_price, self.vehicle_currency)
        except Exception as e:
            logger.error(f"Failed to convert price for clearance ranges: {e}")
            # As a very last resort, return the minimal bracket tax to avoid crashing
