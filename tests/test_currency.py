...
def test_get_rate_passes_day(monkeypatch):
    rates._get_rate.cache_clear()
    conv = MagicMock()
    conv.convert.return_value = 1.23
    monkeypatch.setattr(rates, "_get_converter", lambda: conv)
    day = date(2020, 1, 1)
    rate = rates._get_rate("USD", "EUR", day)
...
def test_to_eur_fallback(code, rate, monkeypatch):
    monkeypatch.setattr(
        rates, "_get_rate", MagicMock(side_effect=Exception("fail"))
    )
    assert rates.to_eur(10, code) == pytest.approx(10 * rate)
...
def test_to_rub_fallback(code, rate, monkeypatch):
    ...
    expected = 10 * rate * rates._EUR_TO_RUB
    assert rates.to_rub(10, code) == pytest.approx(expected)
